...
For the measurement process, laboratory component raw sequence data are generated from environmental samples (Fig. \@(fig:measurementProcessLab)).
...
Then PCR product concentrations are normalized to minimize variability in the number of reads obtained per sample.
Finally, the normalized PCR products are pooled and sequenced.
Preferential DNA extraction and PCR amplification are two of the largest sources of bias in the measurement process (Fig. \@(fig:cause-effect)).
...
Computational methods, collectively referred to as the bioinformatic pipeline, convert raw sequence data into an annotated count table for use in downstream analysis (Fig. \@(fig:measurementProcesComp)).
...
The following section provides additional details about the feature inference and normalization methods that are the focus of the assessment framework presented in this disseration.
...
There are four parts to developing measurement assessment methods -
...
Each assessment type can be used to evaluate part of or the whole measurement process.
...
@Schirmer2015 were able to evaluate the impact of different laboratory protocols and pre-processing methods based on error rate reduction.
...
Interlaboratory studies are an example of this type of assessment where multiple laboratories process the same set of samples or dataset using either the same or different protocols.
...
Once the measurand is defined, appropriate data and metrics can be identified.
