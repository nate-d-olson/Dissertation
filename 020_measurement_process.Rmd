...
Additionally, as the 16S rRNA gene is well studied there are a number of extensive well curated databases, e.g. SILVA, RDP, and Greengenes [@quast2012silva;@McDonald2012-ro;@Cole2014].
...
Additionally, the 16S rRNA is a multicopy gene that is known to be horizontally transferred between organisms as a result the within genome 16S rRNA gene copy diversity can be greater than the between genome diversity for some genera [@pei2010diversity].
...
By inferring the true biological sequence the method avoids having to define an arbitrary threshold for grouping sequences.
...
In addition to uncertainty in the unmixed sample values, there maybe uncertainty in the mixture design.
