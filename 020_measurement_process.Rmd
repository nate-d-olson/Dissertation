...
In this section, I will provide an overview of the measurement process, and highlight significant known sources of error (Fig. \@ref(fig:cause-effect)).
...
DADA2 was first published sequence inference method.
It uses a probabilistic model and an expectation-maximization algorithm to test whether less abundant sequences are derived from higher abundance sequences [@callahan2016dada2].
...
When developing an assessment method, one first needs to determine what part of the measurement process one is interested in evaluating.
...
Assessment methods can also evalute part of the measurement process, such as the computational methods or laboratory components.
...
To evaluate sequencing error rates @Schirmer2015 used a mock community of 59 strains and compared the observed sequence data to 16S rRNA sequences from genome sequences for the 59 strains.
...
Interlaboratory studies are an example of this type of assessment where multiple laboratories process the set of samples or dataset using either the same or different protocols.
...
For quantitative measurands, such as count table values, error rate (the difference between the expected and observed values, normalized by the expected value) is commonly used.
