...
Metrology, measurement science, provides a well established framework for characterizing and evaluating measurements [@Magnusson2014-qz; @ellison2013eurachem].
This framework was established for primarily for routine single measurand assay and well defined applications.
For example measurement validation is used to determine if a measurement process is fit for purpose, that is the measurements meet a specific requirement.
The requirements can be measurement bias and precision are within a defined range or a minimum detectable analyte concentration.
Measurement validation requires a sample that is representative of the type of sample the measurement process will encounter.
The sample must also have known charateristics for use in evaluating the measurement results.
Additionally, the measurement process uncertainty is also determined.
The measurement uncertainty is obtained by first defining the measurand and identifying sources of uncertainty [@ellison2013eurachem].
...
The expanded uncertainty is then calculated by combining uncertainty estimates for individual sources.
The observed measurement value and its expanded uncertainty along with the measurement requirements are then used to validate the measurement process
...
Measurement process assessment consists of three steps; decomposing the measurement process, designing experiments to islolate measurement process elements, and evaluating bias and dispersion at each element.
In this chapter I will describe and decompose the 16S rRNA sequencing measurement process.
A measurement process is assessed relative to the measurand, the entity quantified by the measurement process.
After decomposing the measurement process I will describe common 16S rRNA measurands, feature abundance, relative abundance, as well as alpha- and beta-diversity.
Next, I will describe the steps required to conduct an assessment study.
...
## Designing a Measurement Assessment Experiment

Measurement assessment experiments are used to characterize the bias and dispersion for elements of a measurement process.
...
(3) design an experiment to isolate measurement process element for assessment.
...
The next step is to determine the measurement process steps or element one is interested in evaluating and design an experiment isolating the element.
