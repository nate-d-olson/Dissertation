...
```{r measurementProcess, echo = FALSE, fig.cap = "Diagram of the 16S rRNA marker gene survey measurement process for a simplified example study comparing case and control treatment groups. During _Sample Collection_, samples are obtained from study participants. Colored boxes, representing the collected samples, contains the true microbial community composition indicated by cartoon microbial cells. For the cartoon cells, cell walls and 16S rRNA sequence (grey oval with colored bar) indicate biologically relevant units. DNA extraction is used to isolate DNA from the cellular material, and extraction efficiency can vary by cell wall. Extraction efficiency differences are indicated by differences in the number of red, yellow, and blue bars in the DNA extract compared to the samples. PCR is used to target the 16S rRNA gene. Amplification efficiency biases result in differences in the proportion of red, yellow, and blue PCR products from the DNA extract. Unique sample barcodes added during PCR are indicated as teal and orange bars on left side of the PCR amplicons. Chimeras, a PCR artifact, are indicated as half blue and yellow PCR products. Sequencing libraries are produced by _Pooling_ PCR products from the two samples. Uneven pooling can result in the under-representation of a sample (teal in this example) in the sequence dataset. The resulting libraries are sequenced, introducing additional artifacts indicated as grey reads, for failed sequences, and grey vertical lines for single base sequencing errors. Pre-processing assigns sequencing reads to samples using the unique barcodes and removes low quality reads as well as filters chimeras. Feature inference is used to group the pre-processed reads into biologically relevant units. Sequencing errors can result in spurious features if not assigned to the feature of which it is an artifact of. The yellow and blue sequences with a grey vertical line are spurious features. Finally, taxonomic assignment is performed as part of feature annotation. Spurious features can be assigned to the wrong organism, yellow with grey line, or be unassigned, blue with grey line."}
knitr::include_graphics("measurement_process_draft.png")
```

```{r cause-effect, echo=FALSE, fig.cap = "Cause-effect diagram for the 16S rRNA sequencing measurement process. Causes of bias and variability are indicated as branches, e.g. cell type, and different sources of error are indicated by branch labels, e.g. storage conditions."}
...
Additionally, as the 16S rRNA gene is well studied there several extensive well curated databases, e.g. SILVA, RDP, and Greengenes [@quast2012silva;@McDonald2012-ro;@Cole2014].
There are drawbacks to using the 16S rRNA gene as well.
Due to the conserved nature of the gene, sequences cannot be used for strain level taxonomic classification required for some applications, such as pathogen detection [@janda200716s].
Additionally, the 16S rRNA is a multicopy gene that is known to be horizontally transferred between organisms and, as a result, the within-genome 16S rRNA gene copy diversity can be greater than the between-genome diversity for some genera [@pei2010diversity].


For the measurement process, laboratory component raw sequence data are generated from environmental samples.
Samples are initially collected and stored in a manner that minimizes changes to the community composition prior to sample processing.
...
Computational methods, collectively referred to as the bioinformatic pipeline, convert raw sequence data into an annotated count table for use in downstream analysis.
...
Pre-processing includes initial quality control steps and prepares reads for feature inference.
...
Though taxonomic variability in 16S rRNA sequence diversity [@pei2010diversity], and sequencing errors [@Huse2010;@Schirmer2015;@Amore2016] challenges the validity of these values [@edgar2017updating].
...
Reads are first clustered using closed-reference clustering, and then reads that are not assigned to reference clusters are clustered _de novo_.
...
DADA2, the first published sequence inference method, uses a probabilistic model and an expectation-maximization algorithm to test whether less abundant sequences are derived from higher abundance sequences, and estimates the sequence error profile [@callahan2016dada2].
...
These methods were originally developed for normalizing microarray and RNAseq data but have been successfully used to normalize 16S rRNA marker-gene survey data [@McMurdie2014].
...
(3) define the measurand, and
(4) identify appropriate data and metrics for the assessment.


When developing an assessment method, one needs to first determine what part of the measurement process one is interested in evaluating.
...
Next, one must determine the type of measurement assessment.
Each assessment type can be used to evaluate part of or the measurement process as a whole.
The first assessment type is measurement process performance, or how well observed values agree with expected results.
...
For example, to evaluate the performance of different normalization methods on differential estimates, @McMurdie2014 simulated feature differential abundances by perturbing count table values to have defined effect sizes between treatment and control conditions.
...
The measurement assessment is dependent on the measurand definition.
Once the measurand is defined, appropriate data and metrics can be selected for use in measurement assessment.
...
Another metric for quantitative measurands, is signal to noise ratio, for example, with beta diversity metrics, the ratio for beta diversity between treatments and technical replicates.
...
Limitations to using environmental samples and mock communities for measurement assessment can be addressed by using mixtures of environmental samples.
...
Simulated mixtures of environmental samples (simulated count tables) have previously been used to evaluate deferentially abundance features and beta  [@McMurdie2014].
...
Using the same measurement process and technical replicates to obtain values for the unmixed samples allows for the quantification of the measurement uncertainty but provides no information about potential measurement biases.
In addition to uncertainty in the unmixed sample values, there may be uncertainty in the mixture design.
