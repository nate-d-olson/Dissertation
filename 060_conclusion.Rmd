...
The framework utilizes novel statistical methods in conjunction with a mixture dataset.
...
The assessment framework and dataset presented here provide complementary methods for use in optimizing existing and benchmarking new pipelines and normalization methods.
...
Based on lessons learned from this dissertation new mixture datasets can be developed for further assessment of microbiome measurements.
Additional 16S rRNA sequencing mixture datasets would serve as a complementary resource for the community.
Based on the lessons learned from the development of our two-sample titration mixture dataset.
Using samples with either better characterized prokaryotic DNA proportions or minimal non-prokaryotic DNA would reduce the expected value uncertainty.
Also using samples with greater diversity to generate the mixtures would provide a larger set of features for use in assessment.
As the mixture dataset was only processed using a single laboratory protocol, a set of mixture samples can be used as part of an interlaboratory study for further characterization of the measurement process repeatability and reproducibility.
Finally, similarly, mixtures can be used to assess other microbiome measurement processes such as shotgun metagenomics, metatranscriptomics, and even non-nucleic acid measurements such as metaproteomics and metametabolomics.
