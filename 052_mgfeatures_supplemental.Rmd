...
Here we demonstrate how `metagenomeFeatures` and the `MgDb` annotation packages can be used to characterize taxonomic resolution for a specific clade and amplicon region, specifically for the _Paenibacillus_ genus and V12 and V4 regions. Originally classified under the _Bacillus_ genus, a novel genus was formed based on the 16S rRNA gene similarity in the 1990s. _Paenibacillus_ spp. are facultative anaerobic bacteria present in a variety of environments including the soil, water, and can act as opportunistic pathogens in humans [@ouyang2008]. _Paenibacillus_ spp. will play an important role in sustainable agricultural industries [@grady2016]. As such, appropriate speciation is of interest.
...
In order to differentiate between species the pairwise distances for within species amplicon sequences must be less than the between species distances.
...
#### V12
Extracting the V12 region from the database sequences, only sequences with the expected amplicon length of 318 base pairs are included in the analysis.
...
These are likely partial sequences or sequences with mutation in the primer binding regions.
The R package `PrimerMatch` (__REF__) can be used evaluate mismatches in the primer binding regions.
Here we will limit our taxonomic resolution analysis to trimmed sequences that are consistent with the expected amplicon length of 318 bp and 252 bp for the V12 and V4 regions respectively.

```{r trimInfo, fig.cap = "Primer trimmed sequence length and start and end positions relative to full length sequences for the V12 and V4 regions."}
