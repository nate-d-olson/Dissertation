## Abstract
We developed the metagenomeFeatures R Bioconductor package along with annotation packages for the three primary 16S rRNA databases (Greengenes, RDP, and SILVA) to facilitate working with 16S rRNA sequence databases and marker-gene survey feature data. The metagenomeFeatures package defines two classes, `MgDb` for working with 16S rRNA sequence databases, and `mgFeatures` for working with marker-gene survey feature data. The associated annotation packages provide a consistent interface to the different 16S rRNA databases facilitating database comparison and exploration. The `mgFeatures` represents a key step in the development of a common data structure for working with 16S marker-gene survey data in R.
...
A key step in 16S rRNA marker-gene surveys is comparing representative sequences  to a reference database for taxonomic classification or phylogenetic placement [@nguyen2014tipp]. There are numerous 16S rRNA reference databases of which Greengenes, RDP, and SILVA are arguably the most commonly used [@desantis2006greengenes;@Cole2014;@quast2012silva;@McDonald2012-ro].  Additionally, there are smaller system-specific databases such as HOMD for the human oral microbiome [@chen2010-dw,http://www.homd.org/] and soil reference database [@choi2017-hv]. System-specific databases can improve taxonomic assignments for microbial communities not well represented in the major databases [@rohwer2017-fz].
...
As the 16S rRNA databases contain hundreds of thousands to millions of sequences, an SQLite database is used to store the taxonomic and sequence data. Using an SQLite database prevents the user from loading the full database into memory. The database connection is managed using the RSQLite R package [@muller2017-ju]). The taxonomic data are accessed using the dplyr and dbplyr packages [@wickham2017-ed;@wickham2017-gv]. The DECIPHER package is used to format the sequence data as an SQLite database [@DECIPHER]. The phylo class, defined in a APE R package, is used to define the tree slot [@paradis2004-do]. We developed Bioconductor annotation packages for commonly used databases, Greengenes, RDP, and SILVA (__Table REF__) [@Cole2014;@quast2012silva;@desantis2006-pa]. Along with database specific sequence identifiers RNAcentral identifiers are included in the SQLite table for inter-database comparisons.
