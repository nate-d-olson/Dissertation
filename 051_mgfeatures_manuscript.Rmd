## Abstract
We developed the metagenomeFeatures R Bioconductor package along with annotation packages for the three primary 16S rRNA databases (Greengenes, RDP, and SILVA) to facilitate working with 16S rRNA sequence databases and marker-gene survey feature data. The metagenomeFeatures package defines two classes, \tt{MgDb} for working with 16S rRNA sequence databases, and \tt{mgFeatures} for working with marker-gene survey feature data. The associated annotation packages provide a consistent interface to the different 16S rRNA databases facilitating database comparison and exploration. The \tt{mgFeatures} represents a key step in the development of a common data structure for working with 16S marker-gene survey data in R.
...
\tt{mgFeatures-class} is used for storing and working with marker-gene survey feature data.  Similar to the \tt{MgDb-class}, the \tt{mgFeatures-class} has four slots, for taxonomy, sequences,  phylogenetic tree and metadata. As the number of features in a marker-gene survey dataset is significantly smaller than the number of sequences in a reference database, \tt{mgFeatures-class} uses common Bioconductor data structures, DataFrames and DNAStringSets to define the taxonomic and sequence slots (Pagès et al. 2008; Pagès et al. 2017). Similar to \tt{MgDb-class}, a phylo class object is used to define the tree slot. For both the \tt{MgDb} and \tt{mgFeatures} classes the tree slot is optional and the metadata are stored as a list.
...
Creating a \tt{MgDb} class/ package for ecosystem specific database, e.g. oral microbiome.
